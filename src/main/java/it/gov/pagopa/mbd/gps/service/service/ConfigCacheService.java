 ...
 * It retrieves and updates the cache from the API when necessary, ensuring thread safety and consistency.
 */
@Slf4j
@Service
...
public class ConfigCacheService {
    ...
    /**
     * This method is called when the application is ready. It attempts to retrieve the cache for the first time.
    ...
    /**
     * Retrieves the current cache of creditor institutions. If the cache is not available, it attempts to refresh it.
     * If the cache is still not available after the refresh attempt, it throws an AppException.
     *
    ...
    /**
     * Checks if the cache needs to be refreshed based on the provided event. If a refresh is needed, it retrieves the latest
     * configuration data from the API and updates the cache.
     *
    ...
    public CacheSnapshot checkAndUpdateCache(CacheUpdateEvent event) {
        ...
        try {
            ...
            if (response != null && response.getCreditorInstitutions() != null) {
                String incomingCacheVersion = event != null ? event.getCacheVersion() : (current != null ? current.cacheVersion : null);
                String incomingEventVersion = event != null ? event.getVersion() : (current != null ? current.eventVersion : null);
    ...
    private boolean needsRefresh(CacheSnapshot current, CacheUpdateEvent evt) {
        ...
        if (evt == null) return false; // Se non c'è l'evento e la cache ha già dati, non serve rinfrescare
        if (current.cacheVersion == null) return true;
        if (evt.getCacheVersion() == null || !evt.getCacheVersion().equals(current.cacheVersion)) return true;
