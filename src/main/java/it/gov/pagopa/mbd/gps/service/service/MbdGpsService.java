...
/**
 * Service class responsible for handling MBD payment options and creating debt positions in the GPD system.
 */
@Service
...
public class MbdGpsService {
     ...
     *
     * @param request The MBD payment option request containing the necessary properties for creating the debt position.
    ...
    public String createDebtPosition(MbdPaymentOptionRequest request) {
        ...
        if (creditor == null) {
            throw new AppException(AppError.CREDITOR_INSTITUTION_NOT_FOUND, "Creditor Institution not registered in api-config");
        }

        var response = noticeNumberGeneratorService.generateNoticeNumber(ciFiscalCode);
        var mappingRequest = buildPaymentPositionRequest(requestProperties, creditor.getBusinessName(), response.getNoticeNumber());

        var gpdResponse = gpdClient.createDebtPosition(requestProperties.getCiFiscalCode(), mappingRequest, true);
     ...
     * @return A PaymentPositionModelV3 object representing the debt position to be created in the GPD system.
     */
    private PaymentPositionModelV3 buildPaymentPositionRequest(MbdPaymentOptionRequestProperties requestProperties, String businessName, String nav) {
        ...
        paymentPosition.setIupd(String.format("%s%d_%s", MBD_PREFIX, LocalDate.now(ROME_ZONE_ID).getYear(), nav));
        ...
        debtorModel.setFullName(StringUtils.isEmpty(requestProperties.getDebtorName())
                ? requestProperties.getDebtorSurname()
                : String.format("%s %s", requestProperties.getDebtorName(), requestProperties.getDebtorSurname()));
        ...
        transfer.setRemittanceInformation(String.format(REMITTANCE_INFORMATION_PATTERN, nav, debtorFiscalCode, remittanceInformation));
        transfer.setCategory(category);
        transfer.setStamp(Stamp.builder()
